...
HTMLCanvasElement, fontHeight, SymbolMorph, localize, SpeechBubbleMorph, isNull,
...
PushButtonMorph.prototype.labelShadowOffset = new Point(1.5, 1.5);
...
DialogBoxMorph.prototype.generateKey = function () {
...
DialogBoxMorph.prototype.inform = function (
    ...
    txt.mouseMove = null;
    txt.rootForGrab = function () {
        return this.parentThatIsA(DialogBoxMorph);
    }
...
DialogBoxMorph.prototype.prompt = function (
    ...
    if (isNumeric) {
        ...
        if (!isNull(sliderMin) && !isNull(sliderMax)) {
            sld = new SliderMorph(
                sliderMin * precision,
                sliderMax * precision,
    ...
    if (this.action) {
        if (typeof this.target === 'function') {
            ...
            if (typeof this.action === 'function') {
                ...
                this.target[this.action](this.getInput(), this);
            }
        }
    }
    this.destroy();
...
DialogBoxMorph.prototype.createLabel = function () {
    ...
    if (this.labelString) {
        ...
        this.label.mouseMove = null;
        this.label.rootForGrab = function () {
            return this.parentThatIsA(DialogBoxMorph);
        }
...
AlignmentMorph.prototype.add = function (aMorph) {
	AlignmentMorph.uber.add.call(this, aMorph);
	this.fixLayout();
	this.changed();
};

AlignmentMorph.prototype.addSpace = function (width = 10) {
    var spacer = new Morph();
    spacer.setWidth(width);
    spacer.setHeight(width);
    spacer.render = nop;
    this.add(spacer);
}

AlignmentMorph.prototype.setSubmorphsHeightToMyHeight = function () {
...
InputFieldMorph.prototype.setFontSize = function (fntSize) {
	this.fontSize = fntSize;
	this.cont.alpha = 0;
    this.cont.fontSize = this.fontSize;
    this.cont.fixLayout();
	this.fixLayout();
}
...
BevelMorph.prototype = Object.create(Morph.prototype);
BevelMorph.prototype.constructor = BevelMorph;
BevelMorph.uber = Morph.prototype;

function BevelMorph() {
    this.init();
}

BevelMorph.prototype.init = function () {
    BevelMorph.uber.init.call(this);
    this.sunColor = WHITE;
    this.moonColor = BLACK;
    this.inset = 5;
};

BevelMorph.prototype.render = function (ctx) {
    //ctx.globalCompositeOperation = "destination-out";
    var w = this.width, h = this.height, grad;
    ...
    grad = ctx.createLinearGradient(w, 0, w - inset + 2, 0);
    grad.addColorStop(0, this.moonColor.toString());
    grad.addColorStop(1, CLEAR.toString());
    ctx.fillStyle = grad;
    ctx.fillRect(w - inset, 0, inset, h);

    grad = ctx.createLinearGradient(0, h, 0, h - inset + 2);
    grad.addColorStop(0, this.moonColor.toString());
    grad.addColorStop(1, CLEAR.toString());
    ctx.fillStyle = grad;
    ...
    grad = ctx.createLinearGradient(0, 0, 0, inset + 2);
    grad.addColorStop(0, this.sunColor.toString());
    grad.addColorStop(1, CLEAR.toString());
    ctx.fillStyle = grad;
...
ButtonMorph.prototype = Object.create(PushButtonMorph.prototype);
ButtonMorph.prototype.constructor = ButtonMorph;
ButtonMorph.uber = PushButtonMorph.prototype;
...
function ButtonMorph(
    ...
    this.labelShadowColor = BLACK;
    this.labelShadowOffset = new Point(1.5, 1.5);
    this.fixLayout();
}
