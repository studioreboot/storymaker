/*
    ...
    (with events, videos, and some tweaks)

    a lively Web-GUI
    inspired by Squeak

    orginally written by Jens Mönig
    jens@moenig.org
    ...
    * included only for demo purposes
    ...
    bare essentials:
        ...
        * a single text entry focus per World
          ...
          (added in)
        * events dispatched with logging support(logging IS slow)
    ...
    VIII. things that have been added (and some notes)
    --------------------------------------------------
    Every morph(i.e. the actual Morph class itself, the World, and the Hand) in 
    this morphic.js file (which I call Improved Morphic JS) has events that other
    morphs can listen too (called listeners, internally). Every event has to be a
    MorphicEvent. These MorphicEvent can be dispatched have more attributes via
    the
    
        addAttribute

    function in the MorphicEvent class. You can dispatch events by morphs by
    calling the
    
        dispatchEvent

    function or you could dispatch events to more specific morphs
    by calling
    
        dispatchChildEvent
        dispatchParentEvent

    in a morph or a EventListener (Morph inherits from a EventListener)
    Listeners have one main attribute

        isIgnoringEvents

    This can be set to true or false and events cannot be fired to
    these EventListeners' if that attribute is true.

    I have endevored in this library for a long while, However I still cannot
    find out why I love this library so much. Is it the Windows? The Morphs?
    I honestly don't know. I even downloaded Squeak!
    ...
    * destroy()                - deletion
    ...
    The event has a 'morph' property for the morph that was grabbed.
    There is also a 'which' property which gives the HandMorph
    ...
    The event has a 'morph' property for the morph that was dropped.
    There is also a 'which' property which gives the HandMorph
    ...
    (9) animations
    ---------------
    MAnimations handle gradual transitions between one state and another over a
    ...
    MAnimations are generic and not limited to motion, i.e. they can also handle
    other transitions such as color changes, transparency fadings, growing,
    shrinking, turning etc.

    MAnimations need to be stepped by a scheduler, e. g. an interval function.
...
function getFunctionNameOf (aFunction) {
    if ("name" in aFunction) {
        return aFunction.name;
    } else {
        var fstr = aFunction.toString();
        return fstr.substring(fstr.indexOf(" ") + 1, fstr.indexOf("(") - 1);
    }
}

function setCursorIcon (icon = "default") {
    document.querySelector("body").style.cursor = icon || "default";
}

function getInheritedClassFromString (stringed) {
    return stringed.substring(1, stringed.indexOf(" "))
}

function captilize (str) {
    var string = str.substring(0, 1).toUpperCase() + str.substring(1, str.length);
    return string;
}

function squeakify(aObject) {
    if (aObject instanceof Array) {
        return "[ " + aObject.join(", ") + " ]";
    } else {
        if (aObject.toString().substring(0, 1) != "[") {
            return aObject.toString();
        } else {
            var stringed = aObject.toString()
            if (aObject instanceof Rectangle) {
                return stringed;
            } else {
                if (stringed.substring(0, 1) == "[" && stringed.indexOf(" ") != -1) {
                    var objectClass = stringed.substring(stringed.indexOf(" ") + 1, stringed.length - 1);
                    var inheritedClass = captilize(getInheritedClassFromString(stringed).substring(0, 1));
                    return 'a ' + objectClass + ' | inherits from ' + inheritedClass;
                } else {
                    if (stringed.substring(0, 1) == 'a') {
                        var inheritsFrom = getFunctionNameOf(window[getFunctionNameOf(aObject.constructor)].uber.constructor);
                        return stringed + " | inherits from " + inheritsFrom;
                    } else {
                        return stringed;
                    }
                }
...
function canBeANumber (str) {
    return /\D/g.test(str.toUpperCase());
}
...
// MAnimation instance creation:

function MAnimation(setter, getter, delta, duration, easing, onComplete) {
...
MAnimation.prototype.start = function () {
...
MAnimation.prototype.step = function () {
...
function Color(r = 0, g = 0, b = 0, a = null) {
...
Color.prototype.toString = function () {
    return `rgba(${Math.round(this.r).toString()}, ${Math.round(this.g).toString()}, ${Math.round(this.b).toString()}, ${Math.round(this.a).toString()})`;
};
...
/*
    I am a GRAPHICAL filter that uses the CanvasRenderingContext2D
    filter attribute. My creator has made it so that it informs you
    if your browser doesn't support it.
*/
...
Filter.prototype.init = function (attr, value) {
    this.name = attr;
    this.value = value;
};

Filter.prototype.toString = function () {
    return `${this.name.toLowerCase()}(${this.value.toString()})`;
};

Filter.prototype.applyFilter = function (ctx, aWorld) {
    if ("filter" in ctx) {
        ctx.filter += this.toString();
    } else {
        aWorld.inform("Cannot apply filters in\nthis browser.\nTry Google Chrome!");
    }
}
...
Rectangle.prototype.highlight = function () {
    this.higlighter = new Morph();
    this.higlighter.setPosition(this.origin, false);
    this.higlighter.setExtent(this.corner, false);
    this.higlighter.setColor(new Color(0, 159, 255, 0.45));
    if (world) {
        world.add(this.higlighter);
    }
};

Rectangle.prototype.unhighlight = function () {
    if (this.higlighter instanceof Morph) {
        this.higlighter.destroy();
        this.higlighter = null;
    }
}
...
Rectangle.prototype.inset = function (delta) {
    this.origin = this.origin.add(delta);
    this.corner = this.corner.subtract(delta);
}
...
Rectangle.prototype.spread = function () {
    ...
    return this.origin.floor().corner(this.corner.ceil()).expandBy(5);
};
...
BaseNode.prototype.clearChildren = function () {
    this.children.forEach((child) => {
        child.parent = null;
    })
    this.children = [];
...
BaseNode.prototype.parentThatIsA = function (...args) {
    // including myself
    // Note: you can pass in multiple constructors to test for
    var i;
    for (i = 0; i < args.length; i += 1) {
        if (this instanceof args[i]) {
            return this;
        }
    }
    if (!this.parent) {
    ...
    return this.parentThatIsA.apply(this.parent, args);
};
...
BaseNode.prototype.childThatIsA = function (...args) {
    // including myself
    // Note: you can pass in multiple constructors to test for
    var i, hit;
    for (i = 0; i < args.length; i += 1) {
    ...
    for (i = 0; i < this.children.length; i += 1) {
        hit = this.childThatIsA.apply(this.children[i], args);
        if (hit) {
            return hit;
        }
    }
...
BaseNode.prototype.siblingThatIsA = function (...args) {
    for (let i = 0; i < this.siblings.length; i++) {
        const sibling = this.siblings[i];
        for (let j = 0; j < args.length; j++) {
            const construc = args[j];
            if (sibling instanceof construc) {
                return sibling;
            }
        }
    }
...
/*
    My name litterly describes ME.

    I inherit from BaseNode inorder to dispatch events to
    other event listeners.
*/

EventListener.prototype = Object.create(BaseNode.prototype);
EventListener.prototype.constructor = EventListener;
EventListener.uber = BaseNode.prototype;
...
EventListener.prototype.init = function () {
    EventListener.uber.init.call(this);
    this._events = {}; // For the addEventListener function
    this.ignoresEvents = false;
};

EventListener.prototype.dispatchEvent = function (ev) {
    var listeners = this.allChildren.filter((child) => {
        return child.ignoresEvents == false;
    })
...
EventListener.prototype.dispatchGlobalEvent = function (ev) {
    this.root.dispatchEvent(ev);
}

EventListener.prototype.dispatchChildEvent = function (ev) {
    var listeners = this.allChildren.filter((child) => {
        return child.ignoresEvents == false;
    });
    listeners.splice(1, listeners.length).forEach((child) => {
        if (!child.ignoresEvents) {
            child.fireEvent(ev);
        }
    });
    ev.dispatched();
...
EventListener.prototype.dispatchParentEvent = function (ev) {
    var listeners = this.allParents.filter((parent) => {
        return parent.ignoresEvents == false;
    });
    listeners.splice(1, listeners.length).forEach((parent) => {
        if (!parent.ignoresEvents) {
            parent.fireEvent(ev);
        }
    });
    ev.dispatched();
...
EventListener.prototype.hasEvent = function (ev) {
    if (!this.ignoresEvents) {
        var onListenerFunc = this[ev.getFullName()]
        if (onListenerFunc instanceof Function) {
            return true;
        }
        if (this._events[ev.getFullName()] instanceof Array) {
            return true;
        }
        return false;
    } else {
...
EventListener.prototype.runEvent = function (event) {
    var onListenerFunc = this[event.getFullName()];
    if (onListenerFunc instanceof Function) {
        onListenerFunc.apply(this, event.getArgumentArray());
    }
    if (this._events[event.eventType] instanceof Array) {
        this._events[event.eventType].forEach((func) => {
            func.apply(this, event.getArgumentArray());
        })
    }
};

EventListener.prototype.fireEvent = function (event) {
    if (showEvents) {
        console.log("Event '" + event.toString() + "' dispatched to a " + getFunctionNameOf(this.constructor) + " from " + getFunctionNameOf(event.which.constructor));
    }
    if ((!event.hasBeenDispatched) && (!this.ignoresEvents)) {
        if (this.hasEvent(event)) {
            this.runEvent(event); // Well. ye.
        } else {
            nop(event.eventType);
        }
    }
...
EventListener.prototype.addEventListener = function (eventType, func) {
    if (!this._events[eventType] instanceof Array) this._events[eventType] = [];
    this._events[eventType].push(func);
    func.etype = eventType; // Identifying functions.
};

EventListener.prototype.on = EventListener.prototype.addEventListener;

EventListener.prototype.deleteEventsForType = function (eventType) {
    this._events[eventType].forEach((func) => {
        func.etype = "";
    })
    this._events[eventType] = [];
};

EventListener.prototype.removeEventListener = function (func) {
    var eventType = func.etype;
    this._events[eventType].splice(this._events[eventType].indexOf(func), 1);
    func.etype = ""; // Remove the event type.
};

EventListener.prototype.off = EventListener.prototype.removeEventListener;

EventListener.prototype.toString = function () {
    return 'a EventListener (' + Object.keys(this._events).length.toString() + ') [ ' + this.depth  + ' | ' + this.allChildren.length + ' ]';
}
...
class MorphicEvent {
    constructor(type, from) {
        if (!from instanceof EventListener)
            throw new Error("Cannot call a MorphicEvent from a Non-EventListener");
        ...
        this.eventPrefix = "on";
        this.eventType = type;
        this.worldEvent = false;
    }

    getFullName() {
        return this.eventPrefix + this.eventType;
    }
    ...
    dispatchTo(aMorph) {
        if (!this.hasBeenDispatched) {
            aMorph.dispatchEvent(this);
        }
    }

    addAttribute(name, value) {
        this[name] = value;
        this.addedAttribs.push(name);
    }

    hasAttribute(name) {
        return not(isNil(this[name]));
    }
    ...
    clearAddedAttribs() {
        this.addedAttribs.forEach((attr) => {
            delete this[attr];
        });
        this.addedAttribs = [];
    }

    isEventTypeOf(eventType) {
    ...
    toString() {
        return `a ${getFunctionNameOf(this.constructor)} (${this.eventType}, ${this.addedAttribs.join()})`;
    }

    getArgumentArray () {
        return this.args.concat(this);
    }

    static new(type, from) {
        return new MorphicEvent(type, from);
    }
};
...
/*
    I am just an event with a prefix of 'reactTo'
*/

class MorphicReactEvent extends MorphicEvent {
    constructor (type, from) {
        super(type, from);
        this.eventPrefix = "reactTo";
    }
}
...
class MorphicMouseEvent extends MorphicEvent {
    constructor (type, from) {
        super(type, from);
        this.eventPrefix = "mouse";
    }
}
...
// Morph inherits from EventListener:

Morph.prototype = Object.create(EventListener.prototype);
Morph.prototype.constructor = Morph;
Morph.uber = EventListener.prototype;
...
Morph.prototype.toString = function () {
    return `a ${getFunctionNameOf(this.constructor)} ${this.children.length.toString()} ${this.bounds.toString()}`
};
...
Morph.prototype.moveBy = function (delta) {
    ...
    var ev = new MorphicEvent("moved", this);
...
Morph.prototype.changeTopBy = function (y) {
    this.setTop(this.top + y);
};

Morph.prototype.changeLeftBy = function (x) {
    this.setLeft(this.left + x);
};

Morph.prototype.changeBottomBy = function (y) {
    this.setBottom(this.bottom + y);
};

Morph.prototype.changeRightBy = function (x) {
...
Morph.prototype.dispatchResized = function () {
    var ev = new MorphicEvent("parentresized", this);
    this.fireEvent(new MorphicEvent("resized", this));
    this.dispatchChildEvent(ev);
}

Morph.prototype.setExtent = function (aPoint, canDispatchEvent = true) {
...
Morph.prototype.setWidth = function (width, canDispatchEvent = true) {
    this.setExtent(new Point(width || 0, this.height), canDispatchEvent);
};

Morph.prototype.setHeight = function (height, canDispatchEvent = true) {
    this.setExtent(new Point(this.width, height || 0), canDispatchEvent);
};
...
Morph.prototype.getEntireImage = function () {
    var thing = newCanvas(this.extent, false, this.cachedImage);
    this.fullDrawOn(thing.getContext("2d"), this.bounds);
    return thing;
}
...
Morph.prototype.clearUndrawablePoints = function (ctx) {
    this.undrawablePoints.forEach((undrawable) => {
        ctx.clearRect(undrawable.left, undrawable.top, undrawable.right, undrawable.bottom);
        this.parent.drawOn(ctx, undrawable, this); // Make it so that the World is always seen.
        this.siblings.forEach((sib) => {
            sib.fullDrawOn(ctx, undrawable);
        })
    });
}
...
Morph.prototype.drawOn = function (ctx, rect) {
    ...
    this.clearUndrawablePoints(ctx);
    ctx.restore();
};

Morph.prototype.fullDrawOn = function (aContext, aRect, exclude) {
    if (!this.isVisible) {return; }
    this.drawOn(aContext, aRect);
    if (exclude instanceof Morph) {
        this.children.filter((child) => {
            return child != exclude;
        }).forEach((child) => {
            child.fullDrawOn(aContext, aRect, exclude);
        });
    } else {
...
Morph.prototype.float = function () {
    this.noDropShadow = true;
    this.addShadow();
    this.fps = 20;
    var oldStep = this.setExtent;
    this.oldExtent = oldStep;
    this.setExtent = function (aPoint) {
        oldStep.call(this, aPoint);
        this.removeShadow();
        this.addShadow();
    }
};

Morph.prototype.unfloat = function () {
    this.removeShadow();
    this.setExtent = this.oldExtent;
}
...
Morph.prototype.dispatchRender = function () {
    var ev = new MorphicEvent("render", this);
    this.dispatchEvent(ev);
}
...
Morph.prototype.changed = function (canDispatch = true) {
    var w = this.root;
...
Morph.prototype.dispatchChanged = function () {
    this.fireEvent(new MorphicEvent("changed", this));
    var childEvent = new MorphicEvent("parentchanged", this);
    this.dispatchChildEvent(childEvent);
}

Morph.prototype.fullChanged = function (canDispatch = true) {
    var w = this.root;
    if (w instanceof WorldMorph) {
        w.broken.push(
...
Morph.prototype.dispatchChildrenUpdate = function () {
    this.dispatchEvent(new MorphicEvent("childchange", this));
};

// Morph accessing - structure:

Object.defineProperty(Morph.prototype, "world", {
    get: function () {
        var root = this.root;
...
Morph.prototype.remove = function (aMorph) {
    aMorph.destroy();
    var ev = new MorphicEvent("childremoved", this);
    this.dispatchEvent(ev);
    this.changed(false);
}
...
Morph.prototype.dispatchCopy = function () {
    var ev = new MorphicEvent("copied", this);
    this.dispatchEvent(ev);
}
 ...
 * duplicated.
 * @param {Map} map - To copy form
 * @returns {Map} copied dictionary.
 */
...
Morph.prototype.wantsDropOf = function (aMorph) {
    ...
    if ((aMorph instanceof HandleMorph) || (aMorph instanceof MenuMorph) || (aMorph instanceof WindowMorph)) {
        return false;
    }
    return this.acceptsDrops;
};

Morph.prototype.dispatchPickedUp = function () {
    var ev = new MorphicEvent("pickedup", this);
    this.dispatchEvent(ev);
}
...
Morph.prototype.slideBackTo = function (
    ...
    this.glideTo(
        ...
        () => {
            ...
            if (this.onDropped) {
                var ev = MorphicEvent.new("Dropped", this.world.hand);
                this.dispatchEvent(ev);
            }
            if (situation.origin.reactToDropOf) {
                var ev = new MorphicReactEvent("DropOf", this);
                ev.addAttribute("morph", this);
                situation.origin.reactToDropOf(ev);
            }
...
Morph.prototype.glideTo = function (endPoint, msecs, easing, onComplete) {
    ...
    world.animations.push(new MAnimation(
        y => this.setTop(y),
        () => this.top,
        -(this.top - endPoint.y),
        msecs === 0 ? 0 : msecs || 100,
...
Morph.prototype.getScreenshot = function () {
    return this.getEntireImage();
}
...
Morph.prototype.resizeToParent = function () {
    this.step = function () {
        this.setExtent(this.parent.extent);
    }
}
...
Morph.prototype.evaluateString = function (code) {
    ...
    try {
        result = Function.apply(null, ["return eval(`" + code + "`);"]).call(this);
        this.changed();
    } catch (err) {
        this.inform(err);
        console.log(err);
...
CursorMorph.prototype.processKeyDown = function (event) {
    ...
    */
    ...
    if (keyName === 'tab' || keyName === 'u+0009') {
        ...
        var trg = new MorphicReactEvent("Keystroke", this);
        trg.pushArgument(event);
        this.target.fireEvent(trg);
    }
...
CursorMorph.prototype.processInput = function (event) {
    ...
    var trg = new MorphicReactEvent("Input", this);
    trg.pushArgument(event);
    this.target.fireEvent(trg);
};
...
BoxMorph.prototype.outlinePath = function (ctx, corner, inset) {
    var w = this.width,
        h = this.height,
        radius = Math.abs(Math.min(corner, (Math.min(w, h) - inset) / 2)),
        offset = radius + inset;

        //var actx = document.createElement("canvas").getContext('2d')
...
BoxMorph.prototype.developersMenu = function () {
    ...
    menu.addItem(
        "toggle top\nflatness",
        () => {
            this.topFlat = !this.topFlat;
            this.changed();
        },
        "toggle the top side's flat"
    )
    menu.addItem(
        "toggle bottom\nflatness",
        () => {
            this.bottomFlat = !this.bottomFlat;
            this.changed();
        },
        "toggle the bottom side's flat"
    )
    menu.addItem(
        "border color...",
        () => {
...
DialMorph.prototype = Object.create(Morph.prototype);
...
SliderMorph.prototype.render = function (ctx) {
    if (this.isFlat) {
        ctx.fillStyle = this.getRenderColor();
        ctx.fillRect(0, 0, this.width, this.height);
        this.button.render = function (actx) {
            var colorBak = this.color;
            if (this.userState === 'highlight') {
                this.color = this.highlightColor;
            } else if (this.userState === 'pressed') {
            ...
            actx.fillStyle = this.getRenderColor();
            actx.fillRect(0, 0, this.width, this.height);
            this.color = colorBak;
        }
    } else {
        SliderMorph.uber.render.call(this, ctx);
    }
}
...
MenuMorph.prototype.shadowBlur = 8
...
StringMorph.prototype.toString = function () {
    ...
    return 'a ' +
        getFunctionNameOf(this.constructor) +
...
StringMorph.prototype.fixLayout = function (justMe) {
    ...
    this.ignoresEvents = !this.isEditable;
};
...
StringMorph.prototype.mouseMove = function (pos) {
    var slot,
        clickedText,
        startMark,
        endMark;
...
StringMorph.prototype.mouseLeave = function () {
    setCursorIcon();
}
...
TextMorph.prototype.mouseMove = StringMorph.prototype.mouseMove;
TextMorph.prototype.mouseLeave = StringMorph.prototype.mouseLeave;
...
TextMorph.prototype.doIt = function () {
    this.receiver.evaluateString(this.text);
    this.edit();
};

TextMorph.prototype.showIt = function () {
    var result = this.receiver.evaluateString(this.text);
    if (!isNil(result)) {
        this.inform(result);
    }
};
...
TriggerMorph.prototype.init = function (
    ...
    this.userState = 'normal'; // 'highlight', 'pressed'
    this.disabledLabelColor = this.labelColor.darker(20);
...
TriggerMorph.prototype.render = function (ctx) {
    var colorBak = this.unpressedColor;
    if (this.userState === 'highlight') {
        this.color = this.highlightColor;
    } else if (this.userState === 'pressed') {
        this.color = this.pressColor;
...
TriggerMorph.prototype.createLabel = function () {
    ...
    if (this.labelString instanceof Morph) {
        this.label = this.labelString;
    } else {
...
TriggerMorph.prototype.updateLabel = function () {
    if (this.isDisabled) {
...
TriggerMorph.prototype.destroy = function () {
    setCursorIcon();
    TriggerMorph.uber.destroy.call(this);
}
...
FrameMorph.prototype.fullDrawOn = function (ctx, aRect, exclude) {
    var shadow, clipped;
    ...
    if (exclude instanceof Morph) {
        this.children.filter((child) => {
            ...
            if (child instanceof ShadowMorph) {
                ...
                child.fullDrawOn(ctx, aRect, exclude);
            }
        });
    } else {
        this.children.forEach(child => {
...
FrameMorph.prototype.tryAdjustingBounds = function () {
    try {
        this.adjustBounds();
    } catch (e) {
        nop(e);
...
FrameMorph.prototype.reactToDropOf = function (e) {
    this.tryAdjustingBounds();
    if (this.parent instanceof ScrollFrameMorph) {
        this.parent.fireEvent(new MorphicEvent("cntschg", this));
    };
};

FrameMorph.prototype.reactToGrabOf = function (e) {
    this.tryAdjustingBounds();
    if (this.parent instanceof ScrollFrameMorph) {
        this.parent.fireEvent(new MorphicEvent("cntschg", this));
    };
...
function ScrollFrameMorph(scroller, size, sliderColor, is3D) {
    this.init(scroller, size, sliderColor, is3D);
};

ScrollFrameMorph.prototype.oncntschange = function () {
    this.adjustScrollBars();
    this.fixLayout();
}

ScrollFrameMorph.prototype.init = function (scroller, size, sliderColor, is3D) {
    ...
    this.vBar.isFlat = !this.is3D;
    this.hBar.isFlat = !this.is3D;
    this.add(this.vBar);
...
IconMorph.prototype = Object.create(Morph.prototype);
IconMorph.prototype.constructor = IconMorph;
IconMorph.uber = Morph.prototype;

function IconMorph (name) {
    this.init(name);
}

IconMorph.prototype.init = function (name) {
    IconMorph.uber.init.call(this);
    this.symName = name;
};

IconMorph.prototype.render = function (ctx) {
    ctx.fillStyle = this.getRenderColor().toString();
    this["render_" + this.symName](ctx);
}

IconMorph.prototype.render_fullscreen = function (ctx) {
...
IconMorph.prototype.render_exit = function (ctx) {
    var w = this.width, h = this.height;
...
IconMorph.prototype.render_exitFullscreen = function (ctx) {
    var w = this.width, h = this.height;
    ctx.fillRect(0, 0, 2, h);
...
IconMorph.prototype.render_minimize = function (ctx) {
    var w = this.width, h = this.height;
    ctx.fillRect(0, h - 3, w, 3);
};

IconMorph.prototype.render_play = function (ctx) {
    var w = this.width, h = this.height;
    ctx.beginPath();
    ctx.lineTo(w, h / 2);
    ctx.lineTo(0, h);
...
IconMorph.prototype.render_pause = function (ctx) {
    var w = this.width, h = this.height;
    var widthofrect = w / 3;
    ctx.fillRect(0, 0, widthofrect, h);
    ctx.fillRect(w - widthofrect, 0, widthofrect, h);
};

IconMorph.addIcon = function (name, func) {
    IconMorph.prototype["render_" + name] = func;
};

IconMorph.prototype.setIcon = function (name) {
    this.symName = name;
    this.changed();
    this.rerender();
}

var LinearGradientMorph;
var RadialGradientMorph;

//////////////////////////////////////////////////////////////////////////
// LinearGradientMorph ///////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////

LinearGradientMorph.prototype = Object.create(Morph.prototype);
LinearGradientMorph.prototype.constructor = LinearGradientMorph;
LinearGradientMorph.uber = Morph.prototype;

function LinearGradientMorph (color1, color2) {
    this.init(color1, color2);
}

LinearGradientMorph.prototype.init = function (color1, color2) {
    LinearGradientMorph.uber.init.call(this);
    this.stop1 = color1 || BLACK.copy();
    this.stop2 = color2 || WHITE.copy();
    this.sides = "h";
};

LinearGradientMorph.prototype.render = function (ctx) {
    var grad = this.createGradientUsing(ctx);
    grad.addColorStop(0, this.stop1.toString());
    grad.addColorStop(1, this.stop2.toString());
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, this.width, this.height);
};

LinearGradientMorph.prototype.createGradientUsing = function (ctx) {
    if (this.sides == "v") {
        return ctx.createLinearGradient(0, 0, 0, this.height);
    } else if (this.sides == "h") {
        return ctx.createLinearGradient(0, 0, this.width, 0);
    } else {
        return ctx.createLinearGradient(0, 0, this.width, this.height);
    }
}

LinearGradientMorph.prototype.developersMenu = function () {
    // 'name' is not an official property of a function, hence:
    var world = this.world instanceof Function ? this.world : this.world,
        userMenu = this.userMenu() ||
            (this.parent && this.parent.userMenu()),
    ...
    menu.addItem(
        ...
        () => {
            this.pickColor(
                menu.title + localize('\nfirst stop color:'),
                this.setFirstStopColor,
                this,
                this.stop1
            );
        },
        'choose another stop color \nfor this morph'
    );
    menu.addItem(
        "second stop color...",
        () => {
    ...
    if (menu.title == "LinearGradientMorph") {
        var txt = this.sides == "v" ? "veritcal" : "horizontal"
        menu.addItem(
            txt,
            () => {
                this.invertSides();
...
LinearGradientMorph.prototype.invertSides = function () {
    if (this.sides == "v") {
        this.sides = "h";
    } else if (this.sides == "h") {
        this.sides = "v"
...
LinearGradientMorph.prototype.setFirstStopColor = function (col) {
    this.stop1 = col.copy();
    this.changed();
};

LinearGradientMorph.prototype.setSecondStopColor = function (col) {
    this.stop2 = col.copy();
    this.changed();
};
...
RadialGradientMorph.prototype = Object.create(LinearGradientMorph.prototype);
RadialGradientMorph.prototype.constructor = RadialGradientMorph;
RadialGradientMorph.uber = LinearGradientMorph.prototype;

function RadialGradientMorph(firstCircleRadius, secondCircleRadius) {
    this.init(firstCircleRadius, secondCircleRadius);
}

RadialGradientMorph.prototype.init = function (firstCircleRadius, secondCircleRadius) {
    this.c1rad = firstCircleRadius || 5;
    this.c2rad = secondCircleRadius || 7;
    RadialGradientMorph.uber.init.call(this);
};
// this does some weird shet
RadialGradientMorph.prototype.render = function (ctx) {
    var center = this.center.copy();
    var scenter = center.subtract(this.position.copy());
    var grad = ctx.createRadialGradient(
        scenter.x - this.c1rad, 
        scenter.y - this.c1rad, 
        this.c1rad, 
        scenter.x - this.c2rad, 
        this.center.subtract(this.position).y - this.c2rad, 
        this.c2rad
    );
    grad.addColorStop(0, this.stop1.toString());
    grad.addColorStop(1, this.stop2.toString());
...
PenFrameMorph.prototype = Object.create(FrameMorph.prototype);
PenFrameMorph.prototype.constructor = PenFrameMorph;
PenFrameMorph.uber = FrameMorph.prototype;

function PenFrameMorph() {
    this.init();
}

PenFrameMorph.prototype.init = function () {
    PenFrameMorph.uber.init.call(this);
    this.pen = new PenMorph();
    this.pen.setCenter(this.center);
    this.add(this.pen);
};

PenFrameMorph.prototype.render = function (ctx) {
    var w = this.width,
        h = this.height;
    ...
    ctx.lineWidth = 1;
    ctx.strokeStyle = new Color(0, 0, 0, 0.9);
    for (let x = 0; x <= Math.floor(this.height); x++) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
    ...
    for (let y = 0; y <= Math.floor(this.width); y++) {
        ctx.beginPath();
        ctx.moveTo(0, y);
...
PhysicMorph.prototype = Object.create(Morph.prototype);
PhysicMorph.prototype.constructor = PhysicMorph;
PhysicMorph.uber = Morph.prototype;

function PhysicMorph () {
    this.init();
}

PhysicMorph.prototype.init = function () {
    PhysicMorph.uber.init.call(this);
    this.vel = new Point();
};

PhysicMorph.prototype.isTouchingOther = function () {
    var sibs = this.siblings;
    for (let i = 0; i < sibs.length; i++) {
        const sibiling = sibs[i];
        if (this.isTouching(sibiling)) {
            return true;
        }
    }
    return false;
...
PhysicMorph.prototype.step = function () {
    
    this.vel.y -= 0.5;
    this.changeTopBy(-this.vel.y);
    if (this.isTouchingOther()) {
        this.vel.y = -this.vel.y;
        this.changeTopBy(-this.vel.y);
        this.vel.y = 0;
    }
    this.vel.x = this.vel.x * 0.8;
    this.changeLeftBy(this.vel.x);
    if (this.isTouchingOther()) {
        this.vel.x = -this.vel.x;
        this.changeLeftBy(this.vel.x);
        this.vel.x = 0;
    }
    if ((this.top > this.parent.height + this.parent.top) || this.bottom > this.parent.bottom) {
        this.keepWithin(this.parent);
        this.vel = ZERO.copy();
    }
}

PhysicMorph.prototype.ondropped = function () {
    this.vel = ZERO.copy();
}

PatternMorph.prototype = Object.create(Morph.prototype);
PatternMorph.prototype.constructor = PatternMorph;
PatternMorph.uber = Morph.prototype;

function PatternMorph () {
    this.init();
}

PatternMorph.prototype.render = function (ctx) {
    var current = false;
    ...
    for (let i = 0; i < 4; i++) {
        ...
        let startX = x;
        for (let z = 0; z < 8; z++) {
            ctx.fillStyle = current == true ? this.color : this.color.darker(20);
...
WindowMorph.prototype = Object.create(BoxMorph.prototype);
WindowMorph.prototype.constructor = WindowMorph;
WindowMorph.uber = BoxMorph.prototype;

function WindowMorph (title = "WindowMorph", dragBarHeight = 16) {
    this.init(title, dragBarHeight);
}

WindowMorph.prototype.init = function (title, drgHeight) {
    WindowMorph.uber.init.call(this);
    this.toString = Morph.prototype.toString;
    this.edge = MorphicPreferences.isFlat ? 0 : 5;
    this.fullChanged();
    ...
    this.builtPanesYet = false;
    this.setExtent(new Point(192, 122));
    this.determineShadow();
    this.noDropShadow = true;
    this.titleTextHeight = 12;
    this.isMinimized = false;
    this.dragBarHeight = drgHeight || 16;

    // gradient things
    this.isGradientalWindow = false;
    this.colorStop1 = WHITE;
    this.colorStop2 = BLACK;
    ...
    this.resizeHandle = null; // BTW: this is hidden.
    ...
    this.savedExtent = this.extent.copy();

    if (this.world instanceof WorldMorph) {
        this.world.fullChanged();
    }
    
    this.buildPanes();
    this.removeUserContextMenus(this);
    this.fixLayout();
};

WindowMorph.prototype.render = function (ctx) {
    if (this.isGradientalWindow) {
        // copied from BoxMorph.prototype.render (with tweaks to support gradients)
        if ((this.edge === 0) && (this.border === 0)) {
            BoxMorph.uber.render.call(this, ctx);
            return;
        }
        ...
        gradient.addColorStop(0, this.colorStop1.toString());
        gradient.addColorStop(1, this.colorStop2.toString());
        ...
        WindowMorph.uber.render.call(this, ctx);
    }
}

WindowMorph.prototype.developersMenu = function () {
    var menu = WindowMorph.uber.developersMenu.call(this);
    menu.addLine();
    menu.addItem(
        "set contents\nbg color",
        () => {
        ...
        "shows a menu inwhich give's you the ability to change my\ncontent's color."
    );
    if (this.isFullscreened) {
        menu.addItem(
            "unfullscreen...",
            "normalizeWindow",
            "exits fullscreen and goes back to the last saved extent and\nposition."
        );
    } else {
        menu.addItem(
            "fullscreen...",
            "fullscreenWindow",
            "fullscreen's the entire window to the world's extent."
        );
    }
    menu.addItem(
        "rename window...",
        ...
        "rename this window's title"
    );
    return menu;
};

WindowMorph.prototype.renameMenu = function (menu) {
    this.prompt(
        menu.title + "\ntitle:",
        this.setTitle,
        this,
...
WindowMorph.prototype.fullscreenWindow = function () {
    this.savedPosition = this.position.copy();
    this.savedExtent = new Point(this.extent.copy().x, Math.max(this.extent.copy().y, 112));
    this.setPosition(ZERO);
    this.setExtent(this.world.extent);
    this.edge = 0;
    this.fullChanged();
    this.isFullscreened = true;
    this.createFullButton();
    this.resizeHandle.minExtent.y = 112
    this.reactToWorldResize = function () {
        this.setExtent(this.world.extent);
        this.fullChanged();
        this.contents.setColor(CLEAR);
    }
    this.contents.show();
    this.isMinimized = false;
}

WindowMorph.prototype.normalizeWindow = function () {
    this.setPosition(this.savedPosition);
    this.setExtent(this.savedExtent);
    this.edge = MorphicPreferences.isFlat ? 0 : 5;
    this.fullChanged();
    this.isFullscreened = false;
    this.createFullButton();
    delete this.reactToWorldResize;
};

WindowMorph.prototype.setBackgroundColor = function (aColor) {
    this.setColor(aColor);
    this.isGradientalWindow = false;
};

Object.defineProperty(WindowMorph.prototype, "backgroundColor", {
    get: function () {
        return this.color
    },
    set: function (col) {
...
WindowMorph.prototype.determineShadow = function () {
    this.removeShadow();
    this.children.filter((morph) => {
        return morph instanceof ShadowMorph;
    }).forEach((morph) => {
        morph.destroy();
    })
    this.addShadow();
...
// Whenever resized by the user, fix how things are shown.
WindowMorph.prototype.setExtent = function (aPoint) {
    this.fixLayout();
    this.determineShadow();
    WindowMorph.uber.setExtent.call(this, aPoint);
};

WindowMorph.prototype.fixLayout = function () {
    if (!(this.dragBar && this.fullButton)) return;
    // Drag Bar
    this.dragBar.setWidth(this.width - 4);
    this.dragBar.setLeft(this.left + 2);
    this.dragBar.setTop(this.top + 2);
    this.dragBar.setHeight(this.dragBarHeight);

    // Window Contents
    if (this.contents instanceof FrameMorph) {
        this.contents.setExtent(this.extent.subtract(new Point(4, this.dragBar.height + 4)));
        this.contents.setPosition(this.dragBar.bottomLeft);
    }
    
    // Interactive Shtuff
    this.resizeHandle.setPosition(this.bottomRight.subtract(this.resizeHandle.extent));
    this.closeButton.setPosition(new Point(
        this.dragBar.left + this.dragBar.width - this.closeButton.width,
        this.dragBar.bottomLeft.subtract(this.closeButton.extent)
    ));
    this.fullButton.setPosition(this.closeButton.position.subtract(new Point(this.fullButton.extent.x, 0)));
    this.miniButton.setPosition(this.fullButton.position.subtract(new Point(this.miniButton.extent.x, 0)));
    this.closeButton.setHeight(this.dragBarHeight);
    this.fullButton.setHeight(this.dragBarHeight);
    this.miniButton.setHeight(this.dragBarHeight);

    // Button BG
    this.buttonBG.setPosition(this.fullButton.position.copy());
    this.buttonBG.setWidth(this.fullButton.width + this.closeButton.width);
    this.buttonBG.setHeight(this.miniButton.height);

    // Title
    this.title.setCenter(this.dragBar.center);
    this.rerender();
    this.determineShadow(); // Reload the shadow if something looks weird
};

WindowMorph.prototype.setTitle = function (title) {
    this.titleText = title;
    this.title.destroy();
    this.createTitle();
    this.title.fullChanged();
    this.title.fixLayout();
}

WindowMorph.prototype.moveToCenterOf = function (aMorph) {
    this.setCenter(aMorph.center);
};

WindowMorph.prototype.addContents = function (aMorph) {
    if (!this.contents instanceof FrameMorph) return;
    if (!aMorph instanceof Morph) return;
    if (!isNil(this.contents)) {
        if (this.contents.contents) {
            this.contents.contents.add(aMorph);
        }
    }
    //this.removeUserContextMenus(this.contents);
    this.add(this.resizeHandle);
};

WindowMorph.prototype.minimize = function () {
    if (!this.isMinimized) {
        this.contents.hide();
        this.setExtent(this.dragBar.extent.add(new Point(0, 4)));
        this.resizeHandle.minExtent.y = this.extent.y;
        this.isMinimized = true;
    }
    this.determineShadow();
}

WindowMorph.prototype.alignContentsToCenter = function () {
    var contents = this.contents;
    contents.children.forEach((child) => {
        child.setLeft(contents.left + (contents.width / 2 - child.width / 2));

    });
};

WindowMorph.prototype.popUpIn = function (thing) {
    thing.add(this);
    this.setCenter(thing.center);
}

WindowMorph.prototype.ondropped = function () {
    this.isDraggable = false;
};

WindowMorph.prototype.buildPanes = function () {
    this.createMainPanes();
    this.createTitle();
    this.createResizer();
    this.createButtons();
    this.builtPanesYet = true;
};

WindowMorph.prototype.createResizer = function () {
    var resize;
    resize = new HandleMorph(this, 192, 112);
    resize.render = function (ctx) {
        nop(ctx);
    }
    resize.setPosition(this.bottomRight.subtract(resize.extent));

    this.add(resize);
    this.resizeHandle = resize;
}

WindowMorph.prototype.createMainPanes = function () {
    var drag, contents, myself = this;
    drag = new LinearGradientMorph();
    drag.startingMousePosition = ZERO.copy();
    drag.startPos = new Point();
    drag.isDragBar = true;
    var boxOutline = BoxMorph.prototype.outlinePath;
    drag.bottomFlat = true;
    drag.render = function (ctx) {
        var grad = this.createGradientUsing(ctx);
        grad.addColorStop(0, this.stop1.toString());
        grad.addColorStop(1, this.stop2.toString());
        ctx.fillStyle = grad;
        ctx.beginPath();
        boxOutline.call(this, ctx, Math.max(myself.edge - myself.border, 0), 0);
        ctx.closePath();
        ctx.fill();
    }

    drag.mouseDownLeft = function (pos) {
        this.parent.isDraggable = true;
    };

    drag.stop1 = new Color(0, 83, 255).copy();
    drag.stop2 = new Color(10, 218, 255).darker(10).copy();

    //drag.hierarchyMenu = nomenu;

    drag.mouseClickLeft = function (pos) {
        this.parent.isDraggable = false;
    };
    drag.fullDrawOn = FrameMorph.prototype.fullDrawOn;

    drag.setWidth(this.width);
    drag.setHeight(16);
    drag.ignoresEvents = false;
    this.dragBar = drag;
    this.add(drag);

    contents = new ScrollFrameMorph();
    contents.contents.setColor(CLEAR);
    contents.setPosition(drag.bottomLeft);
    contents.setExtent(this.extent.subtract(new Point(0, drag.height)));
    contents.topFlat = true;
    contents.setColor(CLEAR);

    contents.reactToDropOf = function (e) {
        if (e.morph instanceof WindowMorph) {
            this.world.add(e.morph);
        }
        var lastDropOf = e.morph.reactToDropOf;
        e.morph.reactToDropOf = function (e) {
            lastDropOf.call(this, e);
            if (e.morph instanceof WindowMorph) {
                this.world.add(e.morph);
            }
        }
    ...
    this.dragBar.partOfWindow = true;
    contents.partOfWindow = true;
    contents.isContents = true;
    contents.ignoresEvents = true;

    this.contents = contents;
    this.add(this.contents);
};

WindowMorph.prototype.createButtons = function () {
    var closeButton, mini, ico, bg, closeIcon;

    bg = new Morph();
    bg.setColor(new Color(40, 40, 40, 0.5));
    bg.alpha = 0.5;
    this.dragBar.add(bg);

    closeIcon = new IconMorph("exit");
    closeIcon.setColor(new Color(255, 0, 0));
    closeIcon.setExtent(new Point(20, 20));
    closeIcon.ignoresEvents = true;

    closeButton = new TriggerMorph(this, "destroy", closeIcon, 14);
    closeButton.setHeight(this.dragBarHeight);
    closeButton.label.setTop(closeButton.label.top - 5);
    closeButton.setWidth(15);
    closeButton.setPosition(new Point(
        this.dragBar.left + this.dragBar.width - closeButton.width,
        this.dragBar.bottomLeft.subtract(closeButton.extent)
    ));
    closeButton.setColor(CLEAR);
    closeButton.highlightColor = new Color(0, 0, 0, 0.5);
    closeButton.unpressedColor = CLEAR;
    closeButton.pressColor = BLACK.copy();
    closeButton.labelColor = WHITE.copy();
    closeButton.createLabel();
    closeButton.fullChanged();
    var now = closeButton.label.top
    closeButton.label.setTop(closeButton.label.top - 0.5);
    closeButton.step = function () {
        closeButton.label.setTop(now - 0.5 + closeButton.top);
    }
    closeButton.partOfWindow = true;
    closeButton.ignoresEvents = false;

    this.closeButton = closeButton;
    this.dragBar.add(this.closeButton);

    this.createFullButton();

    ico = new IconMorph("minimize");
    ico.setExtent(new Point(10, 10));
    ico.setColor(WHITE);
    ico.partOfWindow = true;
    ico.ignoresEvents = true;

    mini = new TriggerMorph(this, "minimize", ico);
    mini.setExtent(this.fullButton.extent);
    mini.setPosition(this.fullButton.position.subtract(new Point(mini.extent.x, 0)));
    mini.setColor(CLEAR);
    mini.unpressedColor = this.closeButton.unpressedColor;
    mini.highlightColor = this.closeButton.highlightColor;
    mini.pressColor = this.closeButton.pressColor;
    mini.labelColor = this.closeButton.labelColor;
    mini.createLabel();
    mini.fullChanged();
    mini.partOfWindow = true;
    mini.ignoresEvents = false;

    bg.setPosition(mini.position);
    bg.setHeight(mini.height);
    bg.setWidth(mini.width * 3);
    bg.partOfWindow = true;
    bg.ignoresEvents = true;
    this.buttonBG = bg;

    this.miniButton = mini;
    //this.dragBar.add(this.miniButton);
};

WindowMorph.prototype.createFullButton = function () {
    var fullButton;
    if (this.fullButton instanceof TriggerMorph) {
        this.fullButton.destroy();
    }

    if (this.isFullscreened) {
        sym = new IconMorph("exitFullscreen")
        fullButton = new TriggerMorph(this, "normalizeWindow", sym);
    } else {
        sym = new IconMorph("fullscreen")
        fullButton = new TriggerMorph(this, "fullscreenWindow", sym);
    }
    sym.setExtent(new Point(10, 10));
    sym.setColor(WHITE)
    fullButton.setExtent(this.closeButton.extent);
    fullButton.setPosition(this.closeButton.position.subtract(new Point(fullButton.extent.x, 0)));
    fullButton.setColor(CLEAR);
    fullButton.unpressedColor = this.closeButton.unpressedColor;
    fullButton.highlightColor = this.closeButton.highlightColor;
    fullButton.pressColor = this.closeButton.pressColor;
    fullButton.labelColor = this.closeButton.labelColor;
    fullButton.createLabel();
    fullButton.fullChanged();
    fullButton.ignoresEvents = true;
    fullButton.partOfWindow = true;
    sym.partOfWindow = true;

    this.fullButton = fullButton;
    this.dragBar.add(this.fullButton);
}

WindowMorph.prototype.createTitle = function () {
    var title, self = this;
    ...
    title = new TextMorph(this.titleText, this.titleTextHeight);
    title.setColor(WHITE);
    title.setCenter(this.dragBar.center);
    title.isEditable = false;
    title.isBold = true;
    title.fullChanged();
    title.fixLayout();
    title.partOfWindow = true;
    title.ignoresEvents = false;
    title.mouseMove = null;
    title.mouseDownLeft = function () {
        self.isDraggable = true;
    }
    title.mouseClickLeft = function () {
        self.isDraggable = false;
    }
    title.rootForGrab = function () {
        self.isDraggable = true;
        return self;
    }

    this.title = title;
    if (title.height > this.dragBar.height) {
        this.dragBar.setHeight(title.height);
        this.dragBarHeight = title.height;
        this.fixLayout();
    }
    this.dragBar.addBack(title);
    this.dragBar.changed();
};

WindowMorph.prototype.removeUserContextMenus = function (aMorph) {
    /*aMorph.userMenu = nomenu;
    aMorph.allChildren.forEach((child) => {
        child.hierarchyMenu = this.hierarchyMenu;
        child.userMenu = nomenu;
    });*/
}
...
// InspectorMorph inherits from WindowMorph:

InspectorMorph.prototype = Object.create(WindowMorph.prototype);
InspectorMorph.prototype.constructor = InspectorMorph;
InspectorMorph.uber = WindowMorph.prototype;
...
InspectorMorph.prototype.init = function (target) {
    ...
    InspectorMorph.uber.buildPanes.call(this);
    this.hasCreatedInheritedThings = false;
...
InspectorMorph.prototype.developersMenu = nomenu;

InspectorMorph.prototype.render = function (ctx) {
    var w = this.width, h = this.height;
    var grad = ctx.createLinearGradient(0, 0, w, h);
    grad.addColorStop(0, BLACK.lighter(40));
    grad.addColorStop(1, BLACK.lighter(20))
    if ((this.edge === 0) && (this.border === 0)) {
        BoxMorph.uber.render.call(this, ctx);
        return;
    }
...
//InspectorMorph.prototype.hierarchyMenu = nop;
...
InspectorMorph.prototype.buildPanes = function () {
    ...
    if (this.builtPanesYet) {
        ...
        if (this.contents instanceof FrameMorph) {
            ...
            this.contents = new ScrollFrameMorph();
            this.contents.setColor(CLEAR);
            this.contents.setPosition(this.dragBar.bottomLeft);
            this.add(this.contents);
        }
        ...
        // label
        this.titleText = this.target.toString();
        ...
        switch (this.showing) {
            case 'attributes':
                attribs = attribs.filter((attr) => {
                    return typeof this.target[attr] != "function"
                });
                break;
            case 'methods':
                attribs = attribs.filter((attr) => {
        ...
        this.list.elements.forEach((el) => {
            el.developersMenu = nomenu;
        });

        //this.list.developersMenu = nomenu;
        ...
        this.addContents(this.buttonSubset);
        ...
        this.addContents(this.buttonInspect);
        ...
        this.addContents(this.buttonEdit);
        this.removeUserContextMenus(this);
    }
    // update layout
    this.fixLayout();
...
InspectorMorph.prototype.fixLayout = function () {
    var x, y, r, b, w, h;
    InspectorMorph.uber.fixLayout.call(this);
    ...
    w = r - x;
    if (this.title.height > (this.height - 50)) {
        this.bounds.setHeight(this.title.height + 50);
    }

    try {
        // list
        y = this.title.bottom + 2;
        w = Math.min(
        ...
        this.buttonInspect.setExtent(new Point(w * 1.6, h));

        // edit button
        x = this.buttonInspect.right + this.edge;
        this.buttonEdit.setPosition(new Point(x, y));
        this.buttonEdit.setExtent(new Point(w * 1.5, h));
    } catch (e) {
...
InspectorMorph.prototype.save = function () {
    var actualText = this.detail.contents.children[0].text.toString()
    var txt = canBeANumber(actualText) ? actualText : `"${actualText}"`,
...
InspectorMorph.prototype.execute = function () {
    var txt = this.detail.contents.children[0].text.toString();
    try {
        this.target.evaluateString(txt);
        this.hasUserEditedDetails = false;
        this.target.fullChanged();
    } catch (err) {
        this.inform(err);
    }
};
...
InspectorMorph.prototype.step = function () {
    if (!this.list && this.contents.contents) {
        this.buildPanes();
    }
    this.updateCurrentSelection();
    var lbl = this.target.toString();
    if (this.title.text === lbl) {return; }
    this.titleText = lbl;
    this.dragBar.removeChild(this.title);
    this.createTitle();
    this.fixLayout();
};
...
/* // commented out due to *dumbness*
//////////////////////////////////////////////////////////////////////////
// ExceptionMorph ////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////

function MakeDeja(aStringMorph) {
    aStringMorph.fontName = "DejaVuSans";
    return aStringMorph;
}

ExceptionMorph.prototype = Object.create(WindowMorph.prototype);
ExceptionMorph.prototype.constructor = ExceptionMorph;
ExceptionMorph.uber = WindowMorph.prototype;

function ExceptionMorph(aException) {
    this.init(aException);
}

ExceptionMorph.prototype.init = function (excep) {
    ExceptionMorph.uber.init.call(this);
    this.excep = excep;
    this.titleText = this.excep.toString();
    this.doneStepped = false;
    this.contents.setColor(WHITE.darker(15));
    this.detailBox = null;
    this.exceptionTitle = null;
    this.showException();
};

ExceptionMorph.prototype.showException = function () {
    this.createExceptionTitle();
    this.createDetail();
};

ExceptionMorph.prototype.fixLayout = function () {
    ExceptionMorph.uber.fixLayout.call(this);
    var lastFlat = MorphicPreferences.isFlat;
    MorphicPreferences.isFlat = true;
    if (this.detailBox instanceof ScrollFrameMorph) {
        this.detailBox.setExtent(this.contents.extent.subtract(new Point(45, 85)));
        this.detailBox.children.filter((child) => {
            return child instanceof SliderMorph
        }).forEach((slider) => {
            slider.changed();
            slider.isFlat = true;
        });
    }
    MorphicPreferences.isFlat = lastFlat;
}

ExceptionMorph.prototype.createExceptionTitle = function () {
    var exTitle;
    
    exTitle = MakeDeja(new StringMorph(this.excep.detail().title, 36));
    exTitle.setPosition(this.contents.position.add(new Point(15, 15)));
    exTitle.step = function () {
        exTitle.fixLayout();
        exTitle.changed();
    }
    this.addContents(exTitle);

    this.exceptionTitle = exTitle;
};

ExceptionMorph.prototype.popUpIn = function (aWorld) {
    aWorld.add(this);
    this.setCenter(aWorld.center);
}

ExceptionMorph.prototype.createDetail = function () {
    var detailBox, detail;

    detailBox = new ScrollFrameMorph();
    detailBox.children.filter((child) => {
        return child instanceof SliderMorph
    }).forEach((slider) => {
        slider.button.is3D = false;
        //slider.isFlat = true;
    });
    detailBox.setPosition(this.exceptionTitle.position.add(new Point(5, 45)));
    detailBox.contents.setColor(CLEAR);
    detailBox.setColor(CLEAR);
    
    detail = MakeDeja(new TextMorph(this.excep.detail().detail, 18));
    detail.setPosition(detailBox.position);
    detail.setColor(this.exceptionTitle.color.lighter(40));
    detail.step = function () {
        this.fixLayout();
        this.changed();
    }
    detailBox.addContents(detail);

    this.detailBox = detailBox;
    this.addContents(this.detailBox);
    this.fixLayout();
}

ExceptionMorph.prototype.step = function () {
    this.title.destroy();
    this.createTitle();
    this.step = nop;
}
...
CanvasMorph.prototype = Object.create(Morph.prototype);
CanvasMorph.prototype.constructor = CanvasMorph;
CanvasMorph.uber = Morph.prototype;

function CanvasMorph(aCanvas) {
    this.init(aCanvas);
}

CanvasMorph.prototype.init = function (canv) {
    CanvasMorph.uber.init.call(this);
    this.canv = canv;
    this.fps = 30
};

CanvasMorph.prototype.render = function (ctx) {
    var ftx = this.canv.getContext("2d");
    var imData = ftx.getImageData(0, 0, Math.max(this.canv.width, 10), Math.max(this.canv.height, 10));
    ctx.putImageData(imData, 0, 0, this.width, this.height);
};

CanvasMorph.prototype.step = function () {
    this.changed();
}
...
VideoFrameMorph.prototype = Object.create(FrameMorph.prototype);
VideoFrameMorph.prototype.constructor = VideoFrameMorph;
VideoFrameMorph.uber = FrameMorph.prototype;

function VideoFrameMorph (src) {
    this.init(src);
};

VideoFrameMorph.prototype.init = function (src) {
    this.src = src;
    this.vid = document.createElement("video");
    this.vid.src = this.src;
    VideoFrameMorph.uber.init.call(this);
}

VideoFrameMorph.prototype.play = function () {
    this.vid.play();
};

VideoFrameMorph.prototype.pause = function () {
    this.vid.pause();
};

VideoFrameMorph.prototype.developersMenu = function () {
    var menu = VideoFrameMorph.uber.developersMenu.call(this);
    menu.addLine();
    menu.addItem(
        "play",
        "play",
...
VideoFrameMorph.prototype.destroy = function () {
    this.vid.pause();
    VideoFrameMorph.uber.destroy.call(this);
}

VideoFrameMorph.prototype.setSource = function (src) {
    this.src = src;
    this.update();
}

VideoFrameMorph.prototype.update = function () {
    this.vid = document.createElement("video");
    this.vid.src = this.src;
}

VideoFrameMorph.prototype.render = function (ctx) {
    ctx.fillStyle = this.getRenderColor().toString();
    ctx.fillRect(0, 0, this.width, this.height);
    ctx.drawImage(this.vid, 0, 0, this.width, this.height);
}

VideoFrameMorph.prototype.step = function () {
    this.changed();
}
...
ConsoleMorph.prototype = Object.create(WindowMorph.prototype);
ConsoleMorph.prototype.constructor = ConsoleMorph;
ConsoleMorph.uber = WindowMorph.prototype;

function ConsoleMorph () {
    this.init();
}

ConsoleMorph.prototype.init = function () {
    ConsoleMorph.uber.init.call(this);
    this.console = null;
    this.titleText = "JavaScript Console";
    this.title.destroy();
    this.createTitle();
...
ConsoleMorph.prototype.fixLayout = function () {
    ConsoleMorph.uber.fixLayout.call(this);
    if (this.console instanceof TextMorph) {
        this.console.setWidth(this.contents.width);
    }
}

ConsoleMorph.prototype.step = function () {
    this.changed();
    if (this.console instanceof TextMorph) {
        this.console.fixLayout();
    }
}

window.evaluateString = function (str) {
    var res;
    try {
        res = Function.apply(null, ['return eval(`' + str + '`);']).apply(window)
    } catch (e) {
        var morph = new Morph()
        window['world'].add(morph);
        morph.inform(e.toString());
        morph.destroy();
        console.log(e);
    }
    return res;
...
ConsoleMorph.prototype.createConsole = function () {
    var console = new TextMorph("")
    console.isEditable = true
    console.setReceiver(window);
    console.setWidth(this.contents.width);
    this.console = console;
    this.addContents(this.console);
    this.contents.mouseClickLeft = function () {
        console.edit();
    }
}

ConsoleMorph.prototype.assignConsoleRoot = function () {
    this.console.root = this.root;
}
...
Object.defineProperty(HandMorph.prototype, "world", {
    get: function () {
        return this.myworld;
    }
})
...
HandMorph.prototype.init = function (aWorld) {
    ...
    var img = new Image();
    img.src = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAAXNSR0IArs4c6QAAAFVJREFUOE+10sEKACAIA9D5/x9deBAiVDYj776WagAWAMOwvNEBrxFyAiPkBmQkAySkAmikAyjk2xfolUaCaJBvIntJQr4APn06RTWsZ4BO0a2LSrEBrtEREcSZ+loAAAAASUVORK5CYII=";
    this.img = img;
    this.createCursor();
};

// HandMorph cursor canvas creation:

HandMorph.prototype.createCursor = function () {
    var cur = document.createElement("canvas");
    this.drawCursor(cur.getContext("2d"));
    cur.style.zIndex = 12;
    cur.style.backgroundColor = "transparent";
    cur.style.background = "transparent";
    cur.width = this.img.width;
    cur.height = this.img.height;
    cur.style.position = "absolute";
    //document.body.appendChild(cur);
    //document.body.style.overflow = "hidden";
    this.cursorCanvas = cur;
};

HandMorph.prototype.moveCursorToMe = function (pos) {
    var worldCanvasPosition = getDocumentPositionOf(this.world.worldCanvas);

    function number2px (n) {
        return Math.ceil(n).toString() + 'px';
    }

    this.cursorCanvas.style.left = number2px(worldCanvasPosition.x + pos.x);
    this.cursorCanvas.style.top = number2px(worldCanvasPosition.y + pos.y);
}
...
HandMorph.prototype.fullDrawOn = function (ctx, rect, exclude) {
    if (!this.cachedFullBounds) {
    ...
    ctx.restore();
    this.undrawablePoints.forEach((rect) => {
        this.world.children.forEach((child) => {
            child.fullDrawOn(ctx, rect);
        })
    })
};

HandMorph.prototype.drawCursor = function (ctx) {
    //var ctx = document.createElement("canvas").getContext("2d");
...
/*
    drag 'n' drop events, method(arg) -> receiver:
        ...
        onDropped(handMorph) -> droppedMorph
...
HandMorph.prototype.openEventTo = function (morph, event, doDispatch = false) {
    //console.log("morph: ", morph);
    //console.log("event: ", event);
    if (doDispatch == true) {
        morph.dispatchEvent(event);
    } else {
        morph.fireEvent(event);
    }
}
...
HandMorph.prototype.grab = function (aMorph) {
    ...
    if (this.children.length === 0) {
        ...
        var ev = new MorphicEvent("hndgrabbed", this)
        ev.addAttribute("morphGrabbed", aMorph);
        this.world.dispatchEvent(ev);

        this.changed();
        if (oldParent && oldParent.reactToGrabOf) {
            var eva = new MorphicReactEvent("GrabOf", this);
            eva.addAttribute("morph", aMorph);
            oldParent.fireEvent(eva);
        }
    }
};
...
HandMorph.prototype.drop = function () {
    ...
    if (this.children.length !== 0) {
        ...
        if (morphToDrop.onDropped) {
            morphToDrop.onDropped(this);
        }
        target.dispatchEvent(new MorphicEvent("mrphdropped", this));
        if (target.reactToDropOf) {
            var eva = new MorphicReactEvent("DropOf", this);
            eva.addAttribute("morph", morphToDrop);
            target.dispatchEvent(eva);
        }
    }
};
...
HandMorph.prototype.processMouseDown = function (event) {
    ...
    if (this.children.length !== 0) {
        ...
        var ev = new MorphicEvent("mousedown", this);
        ev.addAttribute("morph", morph);
        morph.dispatchEvent(ev);
        ...
        if (event.button === 2 || event.ctrlKey) {
            ...
            actualClick = 'DownRight';
        } else {
            this.mouseButton = 'left';
            actualClick = 'DownLeft';
        }
        var event = new MorphicMouseEvent(actualClick, this);
        while (morph.ignoresEvents || !morph.hasEvent(event)) {
            morph = morph.parent;
        }
        event.pushArgument(this.bounds.origin);
        this.openEventTo(morph, event);
    }
};
...
HandMorph.prototype.processMouseUp = function () {
    ...
    if (this.children.length !== 0) {
        ...
        var ev = new MorphicEvent("mouseup", this);
        ev.addAttribute("morph", morph);
        morph.dispatchEvent(ev);
        if (this.mouseButton === 'left') {
            expectedClick = 'ClickLeft';
        } else {
            expectedClick = 'ClickRight';
        ...
        try {
            var event = new MorphicMouseEvent(expectedClick, this)
            while (morph.ignoresEvents || !morph.hasEvent(event)) {
                morph = morph.parent;
            }
            event.pushArgument(this.bounds.origin);
...
HandMorph.prototype.processDoubleClick = function () {
    var morph = this.morphAtPointer();
    var ev = new MorphicEvent("dblclick", this);
    ev.addAttribute("morph", morph);
    morph.dispatchEvent(ev);
    ...
    if (this.children.length !== 0) {
        ...
        var event = new MorphicMouseEvent("DoubleClick", this);
        try {
            while (morph && (morph.ignoresEvents || !morph.hasEvent(event))) {
                morph = morph.parent;
            }
            if (morph) {
                event.pushArgument(this.bounds.origin);
...
HandMorph.prototype.dispatchEvent = HandMorph.prototype.dispatchChildEvent = HandMorph.prototype.dispatchParentEvent = nop;
...
HandMorph.prototype.processMouseMove = function (event) {
    ...
    this.moveCursorToMe(pos);

    var ev = new MorphicEvent("mousemov", this);
        ev.addAttribute("morph", morph);
        // some wEiRd shtuff is happenin' here.
        if (ev instanceof MorphicEvent) {
            this.world.dispatchEvent(ev);
        }
...
HandMorph.prototype.processMouseScroll = function (event) {
    var morph = this.morphAtPointer();
    var ev = new MorphicEvent("scrolled", this);
    ev.addAttribute("morph", morph);
    ev.addAttribute("domevent", event);
    morph.dispatchEvent(ev);
    var mevent = new MorphicMouseEvent("Scroll", this);
    while (morph && morph.ignoresEvents || !morph.hasEvent(mevent)) {
        morph = morph.parent;
    }
    if (morph) {
        mevent.pushArgument((event.detail / -3) || (
        ...
        mevent.pushArgument(event.wheelDeltaX / 120 || 0);
        this.openEventTo(morph, mevent);
    }
};
...
*/
    ...
    var ev = new MorphicEvent("filedropped", this);
    ev.addAttribute("morph", target);
    ev.addAttribute("files", files);
...
WorldMorph.prototype.init = function (aCanvas, fillPage) {
    ...
    this.worldCanvas.style.zIndex = 10;
    this.worldContext = this.worldCanvas.getContext("2d");
    //this.worldCanvas.style.cursor = "none";

    if (!document.querySelector("#morphicstyles")) {
        var style = document.createElement('style');
        style.textContent = `@import url('https://fonts.googleapis.com/css2?family=Courier+Prime:wght@400;700&display=swap');
...
WorldMorph.prototype.fullDrawOn = function (ctx, aRect, exclude) {
    ctx.save();

    //this.applyFilters(ctx);
    WorldMorph.uber.fullDrawOn.call(this, ctx, aRect, exclude);
    this.hand.fullDrawOn(ctx, aRect, exclude);

    ctx.restore();
};

WorldMorph.prototype.addFilter = function (aFilter) {
    this.filters.push(aFilter);
    this.applyFilters();
    this.fullDrawOn(this.worldContext, this.bounds);
};

WorldMorph.prototype.applyFilters = function (ctx) {
    var allFilters = ""
    this.filters.forEach((filter) => {
        allFilters += filter.toString() + " ";
    });
    ctx.filter = allFilters;
}

WorldMorph.prototype.addToBroken = function (aMorph) {
    this.broken.push(aMorph.bounds);
}
...
WorldMorph.prototype.updateBroken = function () {
    ...
    this.broken.forEach(rect => {
        if (rect.extent.gt(ZERO)) {
            this.fullDrawOn(ctx, this.showDrawAreas ? new Rectangle(rect.origin.x - 5, rect.origin.y - 5, rect.corner.x + 5, rect.corner.y + 5) : rect);
        }
    });
    this.broken = [];
};

WorldMorph.prototype.draw = function (img) {
...
WorldMorph.prototype.stepMAnimations = function () {
...
WorldMorph.prototype.dispatchCycle = function () {
    var ev = new MorphicEvent("wrldcycle", this);
    this.dispatchChildEvent(ev);
};

WorldMorph.prototype.showBroken = function (broken) {
...
WorldMorph.prototype.grabCanvasContext = function () {
    return this.worldCanvas.getContext("2d");
}

WorldMorph.prototype.regularCycle = function () {
    var curBroken = this.broken;
    this.stepFrame();
    this.hand.children.forEach((child) => {
        child.stepFrame();
    })
    this.stepMAnimations();
    //this.showBroken();
    this.updateBroken();
    if (this.showDrawAreas) {
        this.showBroken(curBroken);
    }
    this.dispatchCycle();
};

WorldMorph.prototype.wasteCycle = function () {
    var ctx = this.grabCanvasContext();var curBroken = this.broken;
    ctx.clearRect(0, 0, this.worldCanvas.width, this.worldCanvas.height);
    this.stepFrame();
    this.stepMAnimations();
    // never update broken.
...
Rectangle.fromXYWH = function (x, y, w, h) {
    ...
    rect.corner = new Point(w, h);
    
    return rect;
}

Rectangle.addFrom = function (aRect, aImage) {
    aRect.corner.add(aImage.width, aImage.height);
    return aRect;
}

WorldMorph.prototype.doOneCycle = function (dt) {
    if (MorphicPreferences.wasteResources) {
        this.wasteCycle();
    } else {
        this.regularCycle();
    }
    //var ctx = this.worldCanvas.getContext("2d");
    //ctx.save();
    //ctx.translate(this.hand.position.x, this.hand.position.y);
    //this.draw(ctx, this.hand.img);
    //this.prevPosition = this.hand.bounds;
    //ctx.restore();
...
WorldMorph.prototype.fillPage = function () {
    ...
    if (this.worldCanvas.width !== clientWidth) {
        ...
        this.setWidth(clientWidth, false);
    }
    if (this.worldCanvas.height !== clientHeight) {
        this.worldCanvas.height = clientHeight;
        this.setHeight(clientHeight, false);
    }
    ...
    var ev = new MorphicEvent("wrldresize", this);
    this.dispatchChildEvent(ev);
    this.doOneCycle();
};

Object.defineProperties(WorldMorph.prototype, {
    screenData: {
        get: function () {
            var data = this.worldCanvas.getContext('2d').getImageData(0, 0, this.width, this.height);
            return data;
        }
    }
})
...
WorldMorph.prototype.toggleWasting = function () {
    touchScreenSettings.wasteResources = !touchScreenSettings.wasteResources
    standardSettings.wasteResources = !standardSettings.wasteResources;
    MorphicPreferences.wasteResources = standardSettings;
}
    ...
    if (this.isDevMode) {
        ...
        if (MorphicPreferences.wasteResources) {
            menu.addItem(
                "stop redrawing\neverything\nevery frame.",
                'toggleWasting',
                "stops redrawing every single part of the World."
            );
        } else {
            menu.addItem(
                "start redrawing\neverything\nevery frame.",
                'toggleWasting',
                "starts redrawing every single part of the World."
            );
        }
        if (showEvents) {
            menu.addItem(
                ...
                "stops logging events to the console"
            );
        } else {
            menu.addItem(
                ...
                "starts logging events to the console"
            );
        }
        if (this.showDrawAreas) {
            menu.addItem(
                ...
                () => {
                    this.showDrawAreas = false
                    this.worldCanvas.getContext("2d").clearRect(0, 0, this.width, this.height);
                    this.fullDrawOn(this.worldCanvas.getContext("2d"), this.bounds);
                },
                "hides any visible draw areas and refreshs the canvas"
            )
        } else {
            menu.addItem(
                "show draw areas",
                () => {
                    this.showDrawAreas = true
                    this.showBroken(this.broken);
                },
                "shows any draw areas and refreshs the canvas\nWARNING: this will slow down performance depending\non your system.\n(use this to see how much resources your morph's need to render\nmore red boxes in your morph the worse.)"
            )
        }
        if (useBlurredShadows) {
            menu.addItem(
        ...
        menu.addItem(
            "show eval.js",
            "openEval",
            "opens the eval.js console into the Window."
        )
        menu.addLine();
    }
    if (this.isDevMode) {
...
WorldMorph.prototype.openEval = function () {
    var script = document.createElement("script");
    script.src = "https://wordylooo.studioreboot.repl.co/java/eval.js";
...
WorldMorph.prototype.userCreateMorph = function () {
    ...
    menu.addItem("video frame", () => {
        create(new VideoFrameMorph(""));
    })
    menu.addItem("console", () => {
        newMorph = new ConsoleMorph();
        this.add(newMorph);
        newMorph.pickUp(this);
    })
    menu.addLine();
    ...
    menu.addLine();
    menu.addItem("linear gradient", () => {
        create(new LinearGradientMorph());
    })
    ...
    menu.addItem('window with\nvideo frame', () => {
        var wind = new WindowMorph();
        var vid = new VideoFrameMorph("");
        vid.step = function () {
            this.changed();
            if (this.parent) this.setExtent(this.parent.extent);
        }
        vid.setPosition(wind.position);
        wind.addContents(vid);
        create(wind);
    })
...
WorldMorph.prototype.edit = function (aStringOrTextMorph) {
    ...
    if (aStringOrTextMorph.color.eq(WHITE)) {
...
//WorldMorph.prototype.reportException = function (aException) {
//    new ExceptionMorph(aException).popUpIn(this);
//}
...
WorldMorph.prototype.hideCanvas = function () {
    this.worldCanvas.style.display = "none";
}

WorldMorph.prototype.showCanvas = function () {
